...
if __name__ == '__main__':
    ...
    while le.is_connected:
        ...
        if state == 1:
            ...
            if (z < 0.4 and start_looking_for_obstacles):
                print("Land")
                x_land = x
                y_land = y
                state = 2
        ...
        if (state == 2):
            ...
            for i in range(10):
                print("position")
                cf.commander.send_position_setpoint(x_land+0.02 , y_land+0.02, 0.3, 0)
                time.sleep(0.1)
            for i in range(10):
                print("landing")
                print("x = {:.3f}, y = {:.3f}, z = {:.3f} ".format(x, y, z))
                cf.commander.send_hover_setpoint(0, 0, 0, 0.3 * ((10-i)/10))
                if(z > 0.3 * ((10-i)/10) + 0.05): 
                    state = 1
                    print("Detected")
                    #break
                    start_looking_for_obstacles = False
                    t2 = threading.Timer(5, set_start_looking_for_obstacles)
                    t2.start()
                idle(cf, 0.3 * ((10-i)/10), 5)
                time.sleep(0.1)
             
             #cf.high_level_commander.land(0, 10)
        #if range_up < 200:
